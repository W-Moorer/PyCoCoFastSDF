...
# -*- coding: utf-8 -*-
"""
从 gradient_sdf_tools_cpu.py 生成的 *_sdf.npy 与 *_meta.json 中提取零等值面，
计算输入 OBJ 表面到零等值面的单向 Hausdorff 距离，并用 PyVista 交互式可视化：
- 颜色：OBJ 点到零等值面的最短距离（单位：与数据一致）
- 文本：报告 HD (最大距离), HD95 (95分位距离), ASD (平均距离) 等指标

用法：
    python demo_sdf_hausdorff_vis_gradient.py

说明：
- 本脚本按 *_meta.json* 的 `bmin/bmax/voxel_step` 构造 UniformGrid，
...
# ==================== 参数定义 ====================
# 在这里修改参数，无需通过命令行输入
SDF_NPY_PATH = "./gradient_outputs/RecurdynSlaveGearTri_sdf.npy"  # SDF 体素文件路径
META_JSON_PATH = "./gradient_outputs/RecurdynSlaveGearTri_meta.json"  # 元数据文件路径
...
def load_meta(meta_json_path: str) -> Tuple[np.ndarray, np.ndarray, Tuple[float, float, float]]:
    with open(meta_json_path, 'r', encoding='utf-8') as f:
        meta = json.load(f)
    # 梯度SDF的元数据字段名与传统SDF略有不同
    bmin = np.array(meta.get('bmin'), dtype=float)
    bmax = np.array(meta.get('bmax'), dtype=float)
...
def main():
    print("=" * 80)
    print("OBJ 到 SDF 零等值面单向距离评估工具（梯度SDF版本）")
    print("=" * 80)
